...
// @Key: Type is used as a key and contains the Index for the data array and a generation counter
// (to solve ABA problem)
// @INDEX_BIT_COUNT: Number of bits for the index into the index map. This specifies an upper limit
// for the entry count.
// @GENERATION_BIT_COUNT: Number of bits for the generation. If this value is too low, ABA problem
// is more likely to happen.
// @INVALID_KEY: It is guaranteed that this key will never be generated and can safely be used to
// represent something like INVALID.
// @SizeType: Type for the erase-table. The slot-map can not contain more entries than the number of
// elements this number can represent. NOTE: INDEX_BIT_COUNT + GENERATION_BIT_COUNT MUST be equal to
// the amount of bits in Key. (Ensured by static assert)
template<
  typename Value,
  typename Key                      = uint64_t,
  unsigned int INDEX_BIT_COUNT      = 32,
  unsigned int GENERATION_BIT_COUNT = 32,
  Key          INVALID_KEY          = ~0,
...
public:
    ...
    // Generation | Index (Into index array or data array. Into data array if slot is unused to
    ...
    static_assert(
      INDEX_BIT_COUNT <= sizeof(IndexType) * 8,
      "Index bit-count is too high, because SizeType can not represent it. Consider another type "
      "for SizeType or decrease the index bit-count.");
    ...
    static_assert(
      (GENERATION_BIT_COUNT + INDEX_BIT_COUNT) == (sizeof(Key) * 8),
    ...
    ///////////////////////////////////////////////////////////////////////////
    Key push_back(const Value &_Val) { return emplace_back(_Val); }

    Key push_back(Value &&_Val) { return emplace_back(std::move(_Val)); }
    ...
    bool erase(Key key)
    {
        ...
        // Mark new index slot as free by setting it as the next head and alias memory to set it to
        // the next free index
    ...
    Value *at(Key key)
    {
        ...
        if (generation != currentGeneration) return nullptr;
    ...
    {
        if (count > m_Capacity) _Create(count);
    }

    ///////////////////////////////////////////////////////////////////////////
    void clear()
    {
        for (unsigned int i = 0; i < size(); i++) m_Data[i].~Value();
        memset(m_Data, 0, m_Capacity * sizeof(Value));
        memset(m_Erase, 0, m_Capacity * sizeof(SizeType));

        for (unsigned int i = 0; i < m_Capacity; i++) m_IndicesAndGeneration[i] = i + 1;
...
private:
    ...
    // Erase table needed to move the last element into a deleted element slot and patch the index
    // table
    ...
    void _Create(const SizeType newCapacity)
    {
        ...
        }
        {
            ...
            if (m_IndicesAndGeneration)
                memcpy(
                  newIndicesAndGen,
                  m_IndicesAndGeneration,
                  oldCapacity * sizeof(IndexGenType));

            for (SizeType i = oldCapacity; i < newCapacity; i++)
                newIndicesAndGen[i] = static_cast<IndexGenType>(i + 1);

            IndexType headIndex =
              m_Head ? static_cast<IndexType>(m_Head - m_IndicesAndGeneration) : 0;
        ...
        }
        {
            ...
            if (m_Erase) memcpy(erase, m_Erase, oldCapacity * sizeof(SizeType));
    ...
    {    // given oldCapacity and newSize, calculate geometric growth
        ...
        if (geometric < newSize) return newSize;    // geometric growth would be insufficient
    ...
    void _CopyFrom(const TSlotMap &other)
    {
        ...
        memcpy(
          m_IndicesAndGeneration,
          other.m_IndicesAndGeneration,
          m_Capacity * sizeof(IndexGenType));
        m_Head = m_IndicesAndGeneration;
    ...
    inline typename std::enable_if<std::is_move_constructible<T>::value>::type
      _MoveConstructObject(T *dst, IndexType indexToStore, T *src, IndexType indexToMove)
    {
    ...
    inline typename std::enable_if<std::is_trivially_destructible<T>::value>::type
      _DeconstructObject(T *src, IndexType index)
    {
        // Do nothing
    }
